...
from torch.nn.parallel.scatter_gather import scatter_kwargs as scatter_kwargs_base
from torch.nn.modules import Module

__all__ = ["SparseDistributedDataParallel"]
...
class _DDPJoinHook(JoinHook):
    def __init__(self, ddp, divide_by_initial_world_size):
        ...
        assert isinstance(ddp, SparseDistributedDataParallel), (
            "DDP join hook requires passing in a SparseDistributedDataParallel "
...
class SparseDistributedDataParallel(Module, Joinable):
    ...
    by default. Note that ``SparseDistributedDataParallel`` does not chunk or
    ...
    ``SparseDistributedDataParallel`` is proven to be significantly faster than
    :class:`torch.nn.DataParallel` for single-node multi-GPU data
    parallel training.

    To use ``SparseDistributedDataParallel`` on a host with N GPUs, you should spawn
    ...
    to construct this module:
        ...
        >>> )
        >>> model = SparseDistributedDataParallel(model, device_ids=[i], output_device=i)
    ...
    .. note::
        ``SparseDistributedDataParallel`` can be used in conjunction with
    ...
    .. note:: When a model is trained on ``M`` nodes with ``batch=N``, the
        ...
        cases, you can just treat a SparseDistributedDataParallel wrapped model, a
    ...
    .. note::
        If you are using SparseDistributedDataParallel in conjunction with the
        ...
        Example::
            ...
            >>> from torch.nn.parallel import SparseDistributedDataParallel as DDP
            >>> import torch
    ...
    .. note::
        SparseDistributedDataParallel currently offers limited support for gradient
    ...
    .. warning::
        ...
        up your model with ``SparseDistributedDataParallel``. Because, when
        wrapping up your model with ``SparseDistributedDataParallel``, the constructor
        of ``SparseDistributedDataParallel`` will register the additional gradient
    ...
    .. warning::
        Using ``SparseDistributedDataParallel`` in conjunction with the
    ...
    Args:
        ...
        bucket_cap_mb: ``SparseDistributedDataParallel`` will bucket parameters into
        ...
        static_graph (bool): When set to ``True``, DDP knows the trained graph is
                     ...
                     Example::
                         >>> # xdoctest: +SKIP("undefined variables")
                         >>> model_DDP = torch.nn.parallel.SparseDistributedDataParallel(model)
                         >>> # Training loop
                         >>> ...
    ...
    Example::
        ...
        >>> net = torch.nn.parallel.SparseDistributedDataParallel(model)
    """
    ...
    def __init__(
        ...
        if not any(p.requires_grad for p in self._module_parameters):
            if len(self._delay_all_reduce_params):
                ...
                self._log_and_throw(
                    RuntimeError,
                    "SparseDistributedDataParallel is not needed when a module "
        ...
        if len(distinct_device_types) != 1:
            self._log_and_throw(
                ValueError,
                "SparseDistributedDataParallel's input module must be on "
        ...
        if (
            ...
            if device_ids or output_device:
                self._log_and_throw(
                    ValueError,
                    "SparseDistributedDataParallel device_ids and output_device arguments "
        ...
        if check_reduction:
            ...
            warnings.warn(
                "The `check_reduction` argument in `SparseDistributedDataParallel` "
        ...
        for param in self._module_parameters:
            if isinstance(param, torch.nn.parameter.UninitializedParameter):
                self._log_and_throw(
                    RuntimeError,
                    "Modules with uninitialized parameters can't be used with `SparseDistributedDataParallel`. "
    ...
    def _register_delay_all_reduce_hook(
        ...
        print("register_delay_all_reduce_hook")
    ...
    def _setup_in_backward_optimizers(self):
        # print("setup_in_backward_optimizers")
        # called
    ...
    def _fire_reducer_autograd_hook(self, idx, *unused):
        print("fire_reducer_autograd_hook")
        """
    ...
    def _module_wait_for_copy_hook(
        ...
        **kwargs: Any,
        ...
        print("module_wait_for_copy_hook")
    ...
    def _build_params_for_reducer(self):
        # print("build_params_for_reducer")
        # called
    ...
    def _build_debug_param_to_name_mapping(self, parameters):
        # print("build_debug_param_to_name_mapping")
        # called
    ...
    def no_sync(self):
        ...
        Example::
            ...
            >>> # xdoctest: +SKIP("undefined variables")
            >>> ddp = torch.nn.parallel.SparseDistributedDataParallel(model, pg)
            >>> with ddp.no_sync():
            >>>     for input in inputs:
    ...
    def _inside_ddp_forward(self):
        # print("inside_ddp_forward")
        # called
        SparseDistributedDataParallel._active_ddp_module = self
        try:
            yield
        finally:
            SparseDistributedDataParallel._active_ddp_module = None

    def _run_ddp_forward(self, *inputs, **kwargs):
        # print("run_ddp_forward")
        # called
        with self._inside_ddp_forward():
            return self.module(*inputs, **kwargs)  # type: ignore[index]

    def _clear_grad_buffer(self):
        # print("clear_grad_buffer")
    ...
    def forward(self, *inputs, **kwargs):
        # print("forward")
        # called
        with torch.autograd.profiler.record_function("SparseDistributedDataParallel.forward"):
    ...
    def _match_all_reduce_for_bwd_pass(self):
        print("running _match_all_reduce_for_bwd_pass")
        comm_work = []
    ...
    def _match_unused_params_allreduce(self):
        print("running _match_unused_params_allreduce")
    ...
    def join(
        ...
        Example::
            ...
            >>>     model = torch.nn.parallel.SparseDistributedDataParallel(
    ...
    def register_comm_hook(self, state: object, hook: Callable):
        ...
        """
        # print("running register_comm_hook")
    ...
    def _register_fused_optim(self, optim: Type, *args, optim_params=None, **kwargs):
        ...
        Example::
            ...
            >>> torch.distributed.init_process_group(backend='nccl', world_size=4, init_method='...')
            >>> net = torch.nn.parallel.SparseDistributedDataParallel(model, pg)
            >>> lr = 1e-2
            >>> betas = (0.9, 0.99)
            >>> eps = 1e-6
        ...
        """
        # Note: importing in function, otherwise this will cause a circular
        # import as optimizer_overlap module needs to import SparseDistributedDataParallel.
    ...
    def _set_params_and_buffers_to_ignore_for_model(
        ...
        torch.nn.parallel.SparseDistributedDataParallel._set_params_and_buffers_to_ignore_for_model(
            model,
            params_to_ignore
        )
        """
    ...
    def _get_ddp_logging_data(self):
        ...
        This interface can be called after SparseDistributedDataParallel() is
        constructed. It returns a dictionary of logging data. It could help
        for debugging and analysis. The logging data includes SparseDistributedDataParallel
        constructor input parameters, some internal states of SparseDistributedDataParallel
    ...
    def _set_static_graph(self):
        ...
        if self.find_unused_parameters:
            warnings.warn(
                "You passed find_unused_parameters=true to SparseDistributedDataParallel, "
